...
export default function ChatsPreview() {
    ...
    const [selectedChatInfo, setSelectedChatInfo] = useState<ChatInfo | null>(null);
    const auth = useAuth();
    const user = useUser();
    ...
    return (
        <div className="chats-preview" >
            ...
            {!displayChat ? <section>
                ...
                {!loading && !error && chatsPreview.length === 0 && <p>No chats found.</p>}
                {!loading && !error && chatsPreview.length > 0 && (
                    <ul>
                        {chatsPreview.map((chat: ChatPreview) => (
                            <li key={chat.chatId} onClick={() => handleChatClick(chat.chatId)}>
                                <p>{chat.otherUserName} : {chat.role}</p>
                                {chat.latestMessage ? (
